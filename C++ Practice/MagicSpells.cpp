...
void counterspell(Spell *spell) {
    if (Fireball* fireball = dynamic_cast<Fireball*>(spell)) {
        ...
        const int s1 = spellName.size(), s2 = journal.size();
        vector<vector<int>> count(s1+1,vector<int>(s2+1,0)); //2d vector of zeros
        for (int i = 1; i <= s1; ++i) {
            for (int j = 1; j <= s2; ++j) {
                if (spellName[i-1] == journal[j-1])
                    count[i][j] = count[i-1][j-1] + 1; //increase current value with previous as value has been found in both strings
                else
                    count[i][j] = max(count[i-1][j], count[i][j-1]); //get max of previous column and previous row
            }
        }
